...
public class Lexer {
    ...
    /** Información detallada de un error léxico con posición */
    public static class ErrorLexico {
        ...
        public ErrorLexico(String mensaje, int linea, int columna, int longitud) {
    ...
    List<ErrorLexico> erroresLexicosDetalle = new ArrayList<>();
    int actual = 0;
    int linea = 1;
    int columna = 1;
    ...
    public List<ErrorLexico> getErroresLexicosDetalle() {
        return erroresLexicosDetalle;
    }
        ...
        while (!fin()) {
            ...
            if (inicio == actual && !fin()) {
                ...
                if (TABLA_TRANS[Q0][claseActual] == SIN_TRANS) {
                    String msg = "Error lexico en linea " + inicioLinea
                        + ": Caracter '" + cActual + "' no pertenece al alfabeto del lenguaje";
                    erroresLexicos.add(msg);
                    erroresLexicosDetalle.add(new ErrorLexico(msg, inicioLinea, inicioColumna, 1));
                ...
                erroresLexicos.add(msg);
                erroresLexicosDetalle.add(new ErrorLexico(msg, inicioLinea, inicioColumna, 2));
                columna = inicioColumna + 1;
                actual = inicio + 1;
            } else if (actual > inicio) {
                String secuencia = fuente.substring(inicio, actual);
                String msg = "Error lexico en linea " + inicioLinea
                    + ": Secuencia no reconocida en el lenguaje: \"" + secuencia + "\"";
                erroresLexicos.add(msg);
                erroresLexicosDetalle.add(new ErrorLexico(msg, inicioLinea, inicioColumna, secuencia.length()));
                columna = inicioColumna + 1;
                actual = inicio + 1;
            }
        }
    ...
    void emitirTokenUnario(int estado, String lexema, int linea, int columna) {
        switch (estado) {
            ...
            case Q_NOT -> {
                ...
                erroresLexicos.add(msg);
                erroresLexicosDetalle.add(new ErrorLexico(msg, linea, columna, 1));
            }
        }
    }
