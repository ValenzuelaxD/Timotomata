...
public class AppController {
    ...
    private VBox crearPanelEditor() {
        ...
        syntaxEditor.setEditorOnKeyReleased(e -> {
        ...
        syntaxEditor.caretPositionProperty().addListener((obs, oldPos, newPos) -> {
            actualizarStatusCursor();
        });

        panel.getChildren().add(syntaxEditor);
        VBox.setVgrow(syntaxEditor, Priority.ALWAYS);
        return panel;
    }
    ...
    public void analizarCodigo() {
        String codigo = syntaxEditor.getText();
        ...
        List<Lexer.ErrorLexico> erroresLexDetalle = lexer.getErroresLexicosDetalle();
        ...
        List<ErrorSintacticoDetalle> erroresSintDetalle = parser.getErroresSintacticosDetalle();
        ...
        // Combinar errores léxicos y sugerencias como errores léxicos
        List<Lexer.ErrorLexico> erroresLexCompletos = new ArrayList<>(erroresLexDetalle);
        for (String sug : sugerenciasReservadas) {
            // Extraer línea de la sugerencia
            try {
                int idx = sug.indexOf("linea ");
                if (idx >= 0) {
                    String resto = sug.substring(idx + 6);
                    int fin = resto.indexOf(' ');
                    if (fin > 0) {
                        int linea = Integer.parseInt(resto.substring(0, fin));
                        erroresLexCompletos.add(new Lexer.ErrorLexico(sug, linea, 1, 1));
                    }
                }
        ...
        // Aplicar resaltado
        syntaxEditor.aplicarResaltado(codigo, tokens, erroresLexCompletos, erroresSintDetalle);
        ...
        actualizarStatus(tokens.size(), erroresLexCompletos.size(), erroresSintFiltrados.size(),
    ...
    private void actualizarStatusCursor() {
        String texto = syntaxEditor.getText();
        int pos = syntaxEditor.getCaretPosition();
    ...
    private void abrirArchivo() {
        ...
        if (file != null) {
            try (BufferedReader br = new BufferedReader(new FileReader(file))) {
                syntaxEditor.setText(br.lines().collect(Collectors.joining("\n")));
